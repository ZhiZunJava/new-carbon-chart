<template>
  <div class="centerMap">
    <div class="mapTitle">
      <div class="zuo"></div>
      <span class="titleText">数据总览</span>
      <div class="you"></div>
    </div>
    <div class="mapWrap">
      <dv-border-box-13>
...
<script>
import { getEquipmentPowerReportList, getProductPowerReportDay, getProductPowerReportMonth, getProductPowerReportYear, getSaleOverall } from '../../api/modules';
export default {
  data() {
    return {
      timer: null,
  ...
  methods: {
    ...
    //获取图标数据
    async getData() {
      await this.getSaleAmount();
      await this.getCarbonEmission();
      await this.getEquipmentRank();
      this.getTimer();
    },
    //获取销售额
    async getSaleAmount() {
      const [err, res] = await getSaleOverall();
      if (!err && res) {
        let list = [{
          title: '今年累计销售额',
          number: {
    ...
    //获取碳排放
    async getCarbonEmission() {
      const [err, res] = await getProductPowerReportDay();
      if (!err && res) {
        let rows = res.rows;
      ...
      //获取本月累计碳排放
      const [err2, res2] = await getProductPowerReportMonth()
      if (!err2 && res2) {
        let rows = res2.rows;
        if (rows && rows.length > 0) {
      ...
      //获取本年累计碳排放
      const [err3, res3] = await getProductPowerReportYear()
      if (!err3 && res3) {
        let rows = res3.rows;
        if (rows && rows.length > 0) {
    ...
    //获取设备排行
    async getEquipmentRank() {
      const [err, res] = await getEquipmentPowerReportList({
        pageNum: 1,
        pageSize: 10
      });
      if (!err && res) {
